...
\
Let's start with a fresh ASP.NET project by executing ```bash dotnet new webapi``` in a new directory. The starter project has a single endpoint defined in `Program.cs`. We can try that endpoint out in the swagger UI. Start up the service (```bash dotnet run```) and follow the link printed in the terminal. If the page is empty, navigate to `/swagger`. In the swagger UI, expand the `/weatherforecast` endpoint, press "Try it out", then "Execute". This will send an HTTP request to the running ASP.NET service and return made up weather data.
...
2. Add the #link("https://www.nuget.org/packages/Oicana#readme-body-tab")[`Oicana` NuGet package] as a dependency with ```bash dotnet add package Oicana --prerelease```.
3. Read the template file and prepare it for compilation at the beginning of `Program.cs`:
  ...
  \
  #note[Template registration (the ```cs new Template()``` call) compiles the template once in development mode to warm up the Typst cache. This happens at startup and uses the `development` value of the input you defined previously.]
